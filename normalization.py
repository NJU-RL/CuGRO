...
POINTMASS_KEYS = ['states', 'actions', 'next_states', 'deltas']
...
class DatasetNormalizer:
    def __init__(self, dataset, normalizer):
        ...
        self.observation_dim = dataset.data_['states'].shape[1]
        self.action_dim = dataset.data_['actions'].shape[1]

        if type(normalizer) == str:
            normalizer = eval(normalizer)

        self.normalizers = {}
        for key, val in dataset.data_.items():
            try:
    ...
    def normalize(self, x):
        ...
        for i in range(x.shape[1]):
            if self.maxs[i] == self.mins[i]:
                norm_x[:, i] = np.zeros_like(x[:, i])
            else:
                norm_x[:, i] = (x[:, i] - self.mins[i]) / (self.maxs[i] - self.mins[i])
        norm_x = 2 * norm_x - 1
        return norm_x

    def unnormalize(self, x):
        x = (x + 1) / 2
        unnorm_x = np.zeros_like(x)
        for i in range(x.shape[1]):
            unnorm_x[:, i] = x[:, i] * (self.maxs[i] - self.mins[i]) + self.mins[i]
        return unnorm_x
